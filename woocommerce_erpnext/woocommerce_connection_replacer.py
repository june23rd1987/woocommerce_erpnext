...
def link_items(items_list, woocommerce_settings, sys_lang):
	for item_data in items_list:
		...
		if frappe.get_value("Item", {"woocommerce_product_id": item_woo_com_id}):#modified by jups
			#Edit Item
			item = frappe.get_doc("Item", {"woocommerce_product_id": item_woo_com_id})#modified by jups
		else:
			#Create Item
			item = frappe.new_doc("Item")

		item.item_name = item_data.get("name")
		#item.item_code = _("woocommerce - {0}", sys_lang).format(item_data.get("product_id"))#modified - removed
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
	...
	for item in order.get("line_items"):
		...
		found_item = frappe.get_doc("Item", {"woocommerce_product_id": woocomm_item_id})#modified jups - do not modify
