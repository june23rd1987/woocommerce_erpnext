...
def link_items(items_list, woocommerce_settings):
	for item_data in items_list:
		...
		if not frappe.get_value("Item", {"woocommerce_id": item_woo_com_id}):
			item = frappe.new_doc("Item")
			item.item_name = item_data.get("name")
			item.item_code = _("{0}").format(item_data.get("product_id"))
			item.woocommerce_id = item_data.get("product_id")
			item.item_group = _("WooCommerce Products")
			item.ugs = item_data.get("sku")
			item.standard_rate = item_data.get("price")
